...
man_dist <- function(cpg_content, cpg_avg) {
  ...
  for (i in 1:nrow(cpg_content)) {
    abs_dif <- abs(cpg_content[i,] - cpg_avg[1,])
    distances[i] <- sum(abs_dif)
  }
...
type1_dist <- man_dist(full, type1_avg)
type2_dist <- man_dist(full, type2_avg)
...
ggsave(output_location_plot, dist_plot, width = 8, height = 6, dpi = 300)
